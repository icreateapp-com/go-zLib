...
// AddFilter 添加过滤字段
func (q *Query) AddFilter(fields ...string) *Query {
	if q.Filter == nil {
		q.Filter = []string{}
	}

	q.Filter = append(q.Filter, fields...)
	return q
}

// AddInclude 添加关联查询字段
func (q *Query) AddInclude(fields ...string) *Query {
	if q.Include == nil {
		q.Include = []string{}
	}
...
func (q *Query) Clone() Query {
	clone := Query{
		...
		Filter:   make([]string, len(q.Filter)),
		Include:  make([]string, len(q.Include)),
	}
	...
	// 深拷贝 Filter
	copy(clone.Filter, q.Filter)

	// 深拷贝 Include
	copy(clone.Include, q.Include)
